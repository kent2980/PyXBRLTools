...
class XBRLModel(BaseXbrlModel):
    ...
    def __init__(self, xbrl_zip_path, output_path) -> None:
        ...
        self.__label_manager = self._initialize_manager(LabelManager, output_path)
        self.__cal_link_manager = self._initialize_manager(CalLinkManager, output_path)
        self.__def_link_manager = self._initialize_manager(DefLinkManager, output_path)
        self.__pre_link_manager = self._initialize_manager(PreLinkManager, output_path)

    def _initialize_manager(self, manager_class:BaseXbrlManager, output_path):
        try:
    ...
    def label_manager(self):
        if self.__label_manager is None:
            raise XbrlListEmptyError("labelLinkbaseファイルが見つかりません。")
        else:
    ...
    def cal_link_manager(self):
        if self.__cal_link_manager is None:
            raise XbrlListEmptyError("calculationLinkbaseファイルが見つかりません。")
        else:
    ...
    def def_link_manager(self):
        if self.__def_link_manager is None:
            raise XbrlListEmptyError("definitionLinkbaseファイルが見つかりません。")
        else:
    ...
    def pre_link_manager(self):
        if self.__pre_link_manager is None:
            raise XbrlListEmptyError("presentationLinkbaseファイルが見つかりません。")
        else:
