...
def cal_link_parser(get_xbrl_in_edjp, get_output_dir):
    ...
    return CalLinkParser.create(xbrl_file.as_posix(), output_path.as_posix())
...
def def_link_parser(get_xbrl_in_edjp, get_output_dir):
    ...
    return DefLinkParser.create(xbrl_file.as_posix(), output_path.as_posix())
...
def pre_link_parser(get_xbrl_in_edjp, get_output_dir):
    ...
    return PreLinkParser.create(xbrl_file.as_posix(), output_path.as_posix())

def test_Base_link_parser_is_instance_Error(get_xbrl_in_edjp, get_output_dir):
    xbrl_files = Path(get_xbrl_in_edjp).rglob("*cal.xml")
    xbrl_file = next(xbrl_files)
    output_path = get_output_dir / "link"
